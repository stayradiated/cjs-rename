 ...
 *
 * Adds the extension used by `this.from` to a path.
 ...
 * > string : path to a file with extension. e.g `./foo.js`
 */

Rename.prototype._addExtension = function _addExtension (path) {
  if (Path.extname(path).length) return path;
  return path + Path.extname(this.from);
};
...
Rename.prototype._fixExtension = function _fixExtension (path, original) {
  var pathExt = Boolean(Path.extname(path).length);
  var origExt = Boolean(Path.extname(original).length);

  if (pathExt === origExt) {
    return path;
  } else if (origExt) { // add extension
    return path + Path.extname(this.to);
